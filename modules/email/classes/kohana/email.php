...
class Kohana_Email {
	 ...
	 * @return  Email
	 */
	public function message($body, $type = 'text/html')
	{
	 ...
	 * @return  Email
	 */
	public function attach_log($log)
	{
	...
	{
		$email = Email::mailer()->send($this->_message, $failed);
		...
		if($this->_log instanceof Log)
		{
			...
			$data = $this->_message->getHeaders();
			$subject = $controller.' -> '.$action;
			$to = (string) $data->get('To');
			
			if($email)
			{
				$this->_log->add(LOG_MAIL, 'Message ":subject" send to ":to"', array(
					':subject' => $subject,
					':to' => $to
				))->write();
			}
			...
			{
				$this->_log->add(LOG_ERR, 'Message ":subject" was not sent to ":to"', array(
					':subject' => $subject,
					':to' => $to
				))->write();
			}
