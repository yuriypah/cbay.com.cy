...
if ($option->pk() > 0) {
    ?>
    <a href="/backend/options/remove/<?php echo $option->pk(); ?>" class="btn btn-mini btn-danger"
       onclick="return confirm('Подтвердите действие');">Удалить опцию целиком</a>
<?
}
?>
    ...
    <div class="span5">
        <fieldset>
            ...
            <select name="option[ranged]">
                ...
                <option value="2" <? echo $option->ranged == 2 ? " selected='selected' " : "" ?>>Диапазон значений
                </option>
            </select><Br/>
            Минимальное значение:&nbsp; <input value="<?php echo $option->ranged_min ?>" type="text" style="width:60px"
                                               name="option[ranged_min]"/><Br/>
            Максимальное значение: <input value="<?php echo $option->ranged_max ?>" type="text" style="width:60px"
                                          name="option[ranged_max]"/>
        </fieldset>
        <?php echo $values; ?>
    </div>
</div>
